...
PACKAGES=(
    ...
    # Fonts — JetBrains Nerd Font for full glyph support
...
cp "$SCRIPT_DIR/hypr/pyprland.toml"                     "$CONFIG/hypr/pyprland.toml"
...
if [[ -f "$HOME/Pictures/avatar-nobg.png" ]]; then
    ...
    warn "Avatar not found at ~/Pictures/avatar-nobg.png — add manually to ~/.config/fastfetch/avatar.png"
fi
...
# Add eza aliases if not present
if ! grep -q "alias ls=" "$CONFIG/fish/config.fish"; then
    echo "alias ls='eza --icons --group-directories-first --color=auto'" >> "$CONFIG/fish/config.fish"
    echo "alias ll='eza -la --icons --group-directories-first --color=auto'" >> "$CONFIG/fish/config.fish"
    echo "alias lt='eza --tree --icons --level=2'" >> "$CONFIG/fish/config.fish"
fi
...
cp "$SCRIPT_DIR/shell/bashrc"                           "$HOME/.bashrc"
...
# ── 11. Eza aliases for bash ─────────────────────────────
info "Adding eza aliases to bash..."
...
if [[ "$SHELL" != "$(which fish)" ]]; then
    chsh -s "$(which fish)"
    success "Fish set as default shell — takes effect on next login"
else
...
echo "     Enable: GTK, Qt, Hyprland, Kitty, User Templates"
echo "  6. Noctalia Settings → Color Scheme → Templates → Advanced"
echo "     Enable User Templates to theme prompts with wallpaper colors"
echo ""
echo "  Key bindings:"
